...
from ppo_agent_discrete import ActorCritic
import torch
...
def play_curling_discrete(model_1, model_2, env: CurlingEnv, device):
    """Play a game of curling between two discrete-action PPO agents."""
    state, _ = env.reset()
    team1_reward = 0
    team2_reward = 0
    done = False
    while not done:
        ...
        if env.current_team == 0:  # Team 1's turn
            dist, _ = model_1(state_tensor)
            action, _ = model_1.get_action(dist)
        else:  # Team 2's turn
            dist, _ = model_2(state_tensor)
            action, _ = model_2.get_action(dist)
        print(action)
        next_state, reward, done, info = env.step(action)
        team1_reward += reward[0]
        team2_reward += reward[1]
...
if __name__ == "__main__":
    stones_per_team = 5
    file_path_1 = "models/ppo_discrete_selfplay_20000_dist.pth"
    file_path_2 = "models/ppo_discrete_selfplay_20000_dist.pth"
    ...
    model_1 = ActorCritic(STATE_DIM, ACTION_DIM, alpha=3e-4, hidden_size=HIDDEN_DIM, action_bin=(5, 10, 5))
    model_1.load_state_dict(torch.load(file_path_1, map_location=device))
    model_1.to(device)
    model_1.eval()

    model_2 = ActorCritic(STATE_DIM, ACTION_DIM, alpha=3e-4, hidden_size=HIDDEN_DIM, action_bin=(5, 10, 5))
    ...
    game_info = play_curling_discrete(model_1, model_2, env, device)
    print(game_info)
